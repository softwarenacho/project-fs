class Match < ActiveRecord::Base
  ...
  def initialize(local, visitor)
    ...
    @events = {}
    @events["#{@time}.#{@partial_time}"] = "Match starts!" #hashes of match events
    ...
    pp @events
    pp @animation
    puts "MARCADOR LOCAL #{@local_goals} - VISITOR #{@visitor_goals}"
  ...
  def iteration(cp) 
    ...
    if w_a == o_a
      ...
      cp = succession_offense(cp, o_a)
    elsif w_a == d_a
      #consequences of defensive taking control 
      cp = succession_defense(cp, dp, d_a, o_a)
    end
    #a minute passed 
    @partial_time += 1
    if @field > 10
    ...
    @animation["#{@time}.#{@partial_time}"] = [cp[0].name,  @field]
    cp
  end
    ...
    if team == @local
      case @field 
        when 10
          team.each do |member|
            if member[2] == "OFF"
              prob_hash[member] = 70/off_count
            elsif member[2] == "MID"
              prob_hash[member] = 20/mid_count
            elsif member[2] == "DEF"
              prob_hash[member] = 10/def_count
            end
          end
       when 9, 8
        team.each do |member|
          if member[2] == "OFF"
            prob_hash[member] = 60/off_count
          elsif member[2] == "MID"
            prob_hash[member] = 30/mid_count
          elsif member[2] == "DEF"
            prob_hash[member] = 10/def_count
          end
        end
      when 7, 6
        team.each do |member|
          if member[2] == "OFF"
            prob_hash[member] = 50/off_count
          elsif member[2] == "MID"
            prob_hash[member] = 40/mid_count
          elsif member[2] == "DEF"
            prob_hash[member] = 10/def_count
          end
        end
      when 5 
        if @events.values.last.include?("goal")
          team.each do |member|
            if member[2] == "OFF"
              prob_hash[member] = 70/off_count
            elsif member[2] == "MID"
          ...
          team.each do |member|
            if member[2] == "OFF"
              prob_hash[member] = 25/off_count
            elsif member[2] == "MID"
              prob_hash[member] = 50/mid_count
            elsif member[2] == "DEF"  
              prob_hash[member] = 25/def_count
            end
          end
        end
      when 4, 3
        team.each do |member|
          if member[2] == "OFF"
            prob_hash[member] = 10/off_count
          elsif member[2] == "MID"
            prob_hash[member] = 40/mid_count
          elsif member[2] == "DEF"  
            prob_hash[member] = 50/def_count
          end
        end
      when 2, 1
        team.each do |member|
  ...
  def select_defensive_player()
    #an action will be analyzed for the defensive player
    @d_hash.each { |k, v| @d_hash[k] = v*rand(0.85..1.15) }
    @d_hash.each do |k , v|
      if k[3]  == 1
        @d_hash[k] = 0
      end
    end
    dp = @d_hash.select {|k,v| v == @d_hash.values.max }
    dp = dp.keys[0]
  end
  ...
  def defensive_decision(dp)
    hash = {"slide_steal" => 0,"tackle_steal" => 0,"contain" => 0,"pull" => 0}
      ...
      hash["contain"] = contain_prob
      pull_prob = dp[0].zen_aggresive*rand(0.9..1.1)*reverse_v(field_prob)
      hash["pull"] = pull_prob
      d_a = hash.select {|k,v| v == hash.values.max }
      d_a= d_a.keys[0]
  end
  ...
  def array_maker(player,action)
    array = []
    if player[1] == @local[0][2] 
      distance_factor = ((@field * 10) + (player[0].longshots))/2
    else
      distance_factor = (((10-@field) * 10) + (player[0].longshots))/2
    end 
    case action
      when "normal_shot"
      ...
      when "contain"
        ...
        puts "*"*100
        puts "ERROR EN ARRAY MAKER NO RECONOCIO #{action}"
        puts "*"*100
    end
  end
  ...
  def success_rate(hash)
    ...
    s_r = (total/100.00)*rand(0.75..1.25)
  end
  ...
  def define_winning_action(dp, cp, d_a, o_a)
    ...
    @o_sr = success_hash(v_array_o)*rand(0.75..1.25)
    @d_sr = success_hash(v_array_d)*rand(0.75..1.25)
  ...
  def gk_succession(cp)
    ...
    op = cp
    cp = select_defensive_player()
    team = local_or_visitor?(cp[1])
    gp =  team[0]
    @events["#{@time}.#{@partial_time}"] = "#{op[0].name} tried to score but #{gp[0].name}  blocked the shot & passes the ball to #{cp[0].name}!"
    cp
  end

  #define if a shot will end in goal or not
  def shot_action(cp, o_a)
    add_score(cp[1], 2)
  ...
  def select_offensive_player(cp)
    @o_hash = prob_team(cp)
    @o_hash.each do |k, v| 
      if @o_hash[k] == cp
        @o_hash[k] = 0
      else
        @o_hash[k] = v*rand(0.85..1.15)
      end
    end
    @o_hash.each do |k , v|
      if k[3]  == 1
    ...
    cp = @o_hash.select {|k,v| v == @o_hash.values.max }
    cp = cp.keys[0]
  end

  #decides what will happen after action comparison with complex methods
  def succession_offense(cp, o_a)
    ot = local_or_visitor?(cp[1])
    case o_a
      when "normal_pace"
  ...
  def fault?(cp, dp)
    ...
    cp_za = cp[0].zen_aggresive * 100
    s_r = success_hash([reverse_s(cp[0].strength), dp[0].strength, reverse_s(cp[0].balance), dp[0].aggression, reverse_s(cp_za), dp_za])
  end
  ...
  def free_kick_action(cp)
    ...
    normal_shot_prob = reverse_v(cp[0].simple_elegant)*reverse_v(cp[0].individual_collab)*rand(0.85..1)*field_prob
    hash["normal_shot"] = normal_shot_prob*1.5
    chip_shot_prob = cp[0].zen_aggresive*reverse_v(cp[0].individual_collab)*rand(0.85..1)*field_prob
    hash["chip_shot"] = chip_shot_prob
    finesse_shot_prob = normal_shot_prob*1.5
    chip_shot_prob = cp[0].simple_elegant*reverse_v(cp[0].individual_collab)*rand(0.85..1)*field_prob
  ...
  def defensive_steal(dp, d_a, cp, o_a)
    sr = fault?(cp, dp)
    if sr > 95
      cp = card_worthy(dp, d_a, cp)
    elsif sr > 85
  ...
  def succession_defense(cp, dp, d_a, o_a)
    ot = local_or_visitor?(cp[1])
    case d_a
      when "contain"
        add_score(cp[1], 1)
        ...
        cp
      when "pull"
        cp = defensive_steal(dp, d_a, cp, o_a)
      when "block"
        if out_of_field?("d") == true
          @events["#{@time}.#{@partial_time}"] = "#{cp[0].name} attempted #{format(o_a)} but #{dp[0].name} performed #{format(d_a)}"
          cp = out_of_field(dp, "d")
        else
          add_score(cp.team, 1)
          @events["#{@time}.#{@partial_time}"] = "#{cp[0].name} attempted #{format(o_a)} but #{dp[0].name} performed #{format(d_a)}"
          r = rand(0..1)
          r == 0 ? cp = select_offensive_player(ot) : cp = select_defensive_player()
        end      
      when "slide_steal"
        cp = defensive_steal(dp, d_a, cp, o_a)
      when "tackle_steal"
